...
if(True):
    df_clean_1 = pd.read_csv("df_clean_1.csv") # post_id col indicates post level
    df_clean_1 = df_clean_1[["post_id","gun","gun_strict","text_fpath","woman"]] # contains duplicates

    # this is really how i should have been doing things the whole time
    df_clean_1['gun_post'] = df_clean_1.groupby('post_id')['gun'].transform(lambda x: 1 if x.max() == 1 else 0) 
    df_clean_1['gun_strict_post'] = df_clean_1.groupby('post_id')['gun_strict'].transform(lambda x: 1 if x.max() == 1 else 0)

    duplicates = df_clean_1[df_clean_1.duplicated(subset=['post_id'], keep=False)]
    print(duplicates[["post_id","gun","gun_post"]])

    df_clean_1 = df_clean_1.drop_duplicates(subset="text_fpath")

    df_empath = pd.read_csv("empath_gun_test.csv") # merge using text_fpath; everything we want is in text_content
    df_empath = df_empath[["text_content", "text_fpath"]]
    df_empath = df_empath.drop_duplicates(subset="text_fpath")
    print(df_empath.shape[0]) # bigger
    print(df_clean_1.shape[0]) # smaller (but there are like 10,000 posts in here that cannot be found in df_empath which is weird - did fpath change somehow?)
    df = pd.merge(df_clean_1, df_empath, how = "inner", on = "text_fpath", indicator = True)
    print(df.shape[0])

    df.to_pickle("word2vec.pkl")

df = pd.read_pickle("word2vec.pkl")
print(df['woman'].head())
...
#latex_output = generate_latex_table(nn)
#print(latex_output)
